...
if (eventName === 'pull_request_target') {
  try {
    ...
    headSha = payload.pull_request?.head?.sha ?? contextSha;
  } catch {
    headSha = contextSha;
  }
} else {
  headSha = resolvedHeadSha || contextSha;
}
...
console.log(`head_sha=${headSha}, status_context=${statusContext}, is_adhoc_dispatch=${isAdHocDispatch}`);
